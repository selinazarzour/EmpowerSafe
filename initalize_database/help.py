...
def predict_safe_scores(model, YEAR, DOY, DOW, HOUR, LONG, LAT):
    user_values = [[YEAR, DOY, DOW, HOUR, LONG, LAT]]
    safe_score = model.predict(user_values)
    return safe_score[0]

model = prediction_model()
safety_score = predict_safe_scores(model, 2024, 92, 7, 2, -79.4, 43.7)